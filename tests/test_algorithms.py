...
class TestDCTAlgorithm:
    ...
    def test_initialization_invalid_block_size(self) -> None:
        ...
        with pytest.raises(ValueError, match="block_size must be a positive power of 2"):
            DCTAlgorithm(block_size=0)

        with pytest.raises(ValueError, match="block_size must be a positive power of 2"):
            DCTAlgorithm(block_size=7)  # Not power of 2

        with pytest.raises(ValueError, match="block_size must be a positive power of 2"):
            DCTAlgorithm(block_size=-8)

    def test_initialization_invalid_dct_coefficient(self) -> None:
        ...
        with pytest.raises(ValueError, match="dct_coefficient must be between"):
            DCTAlgorithm(dct_coefficient=0)  # Too low

        with pytest.raises(ValueError, match="dct_coefficient must be between"):
            DCTAlgorithm(dct_coefficient=64)  # Too high for 8x8 blocks

    def test_initialization_invalid_quantization_factor(self) -> None:
        ...
        with pytest.raises(ValueError, match="quantization_factor must be positive"):
            DCTAlgorithm(quantization_factor=0)
